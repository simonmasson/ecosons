...
function [ntr, utmCoords, xCoord, yCoord, znCoord]=ec_ops_transects
 ...
 if( iscell(SONAR_DATA) && length(SONAR_DATA)>0 && length(BOTCLASS)>0 )
 ...
 switch(sel)
  ...
  case 2 %BOTCLASS data

   nn=1;
   for sds=1:length(BOTCLASS)
    ll=length(BOTCLASS{sds}.time);
    ntr(nn:nn+ll-1)=sds;
    lat(nn:nn+ll-1)=BOTCLASS{sds}.latitude;
    lon(nn:nn+ll-1)=BOTCLASS{sds}.longitude;
    nn=nn+ll;
   endfor
